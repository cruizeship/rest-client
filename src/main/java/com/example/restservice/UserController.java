  ...
  public Object getUser(@RequestBody UserRequest request) {
    ...
    try {
      ...
      if (request.getQuests() != null && request.getQuests().length > 0) {
        // PostgreSQL JSON function for array containment
        for (int quest : request.getQuests()) {
          sqlAdd.add(String.format("quests @> '[%d]'", quest));
        }
      }
...
public Object createUser(@RequestBody UserRequest request) {
    try {
        ...
        System.out.println(questsJson); // Debugging: Verify JSON format
        ...
        jdbc.update(
            connection -> {
                ...
                ps.setInt(4, points);
                // Set the JSON data as Types.OTHER
                ps.setObject(5, questsJson, Types.OTHER); 
                return ps;
            },
            keyHolder);
